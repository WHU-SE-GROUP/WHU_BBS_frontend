...
<!--              @click="deleteComment(data.id)">{{ $t("common.delete") }}</span>-->
        ...
        <span slot="content">
          ...
          <!-- 机器人回复按钮 -->
          <a class="operate comment-bot" v-if="data.depth < 2" @click="generateBotReply(data.id)">
            <i class="iconfont icon-robot" :style="botLoading ? 'color:' + $store.state.themeColor : 'color: #8a919f;'">
              <small>快速回复</small>
            </i>
          </a>
...
export default {
  ...
  methods: {
    ...
    generateBotReply(commentId) {
      ...
      commentBotService.generateBotReply(commentId, this.$route.params.id)
        .then(() => {
          this.$message.success('机器人回复已生成');
          this.getCommentByArticleId();
          this.botLoading = false;
        })
        .catch(err => {
          this.$message.error(err.desc || '生成回复失败');
          this.botLoading = false;
        });
    },
...
#child-comment .comment-comment, .comment-operate, .comment-bot {
  margin-left: 16px;
}
